...
from stable_baselines.common import explained_variance, ActorCriticRLModel, \
...
class PPO2(ActorCriticRLModel):
    ...
    :param env: (Gym environment or str) The environment to learn from
        (if registered in Gym, can be str)
    :param gamma: (float) Discount factor
    :param n_steps: (int) The number of steps to run
    for each environment per update
        (i.e. batch size is n_steps * n_env where n_env is
        number of environment copies running in parallel)
    :param ent_coef: (float) Entropy coefficient for the loss calculation
    :param learning_rate: (float or callable) The learning rate,
        it can be a function
    :param vf_coef: (float) Value function coefficient for the loss calculation
    :param max_grad_norm: (float) The maximum value for the gradient clipping
    :param lam: (float) Factor for trade-off of bias vs variance
    ...
    :param cliprange: (float or callable) Clipping parameter,
        it can be a function
    :param cliprange_vf: (float or callable) Clipping parameter
        for the value function, it can be a function.
        This is a parameter specific to the OpenAI implementation.
        If None is passed (default), then `cliprange`
        (that is used for the policy) will be used.
        IMPORTANT: this clipping depends on the reward scaling.
        To deactivate value function clipping
        (and recover the original PPO implementation),
    ...
    :param tensorboard_log: (str) the log location for tensorboard
        (if None, no logging)
    :param _init_setup_model: (bool) Whether or not to build the network
        at the creation of the instance
    :param policy_kwargs: (dict) additional arguments
        to be passed to the policy on creation
    :param full_tensorboard_log: (bool) enable additional logging
        when using tensorboard
        WARNING: this logging can take a lot of space quickly
    :param seed: (int) Seed for the pseudo-random generators
        (python, numpy, tensorflow).
        If None (default), use random seed.
        Note that if you want completely deterministic results,
        you must set `n_cpu_tf_sess` to 1.
    :param n_cpu_tf_sess: (int) The number of threads for TensorFlow operations
        If None, the number of cpu of the current machine will be used.
    """

    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01,
                 learning_rate=2.5e-4, vf_coef=0.5, max_grad_norm=0.5,
            ...
            with self.graph.as_default():
                ...
                if issubclass(self.policy, RecurrentActorCriticPolicy):
                    assert self.n_envs % self.nminibatches == 0, (
                        "For recurrent policies, the number of environments run "
                        "in parallel should be a multiple of nminibatches.")
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    neglogpac = train_model.prob_dist.neglogp(self.action_ph)
                    self.entropy = tf.reduce_mean(
                        train_model.prob_dist.entropy())
                    ...
                    self.clipfrac = tf.reduce_mean(tf.cast(tf.greater(
                        tf.abs(ratio - 1.0), self.clip_range_ph), tf.float32))
                    loss = self.pg_loss - \
                           self.entropy * self.ent_coef + self.vf_loss * self.vf_coef
                ...
                with tf.variable_scope("input_info", reuse=False):
                    ...
                    if self.full_tensorboard_log:
                        ...
                        tf.summary.histogram(
                            'advantage', self.advs_ph)
                        tf.summary.histogram(
                            'clip_range', self.clip_range_ph)
                        tf.summary.histogram(
    ...
    def _train_step(self, learning_rate, cliprange, obs, returns, masks,
                    actions, values, neglogpacs, update, writer,
        ...
        :param masks: (np.ndarray) The last masks for done episodes
            (used in recurent policies)
        :param actions: (np.ndarray) the actions
        :param values: (np.ndarray) the values
        :param neglogpacs: (np.ndarray) Negative Log-likelihood probability of Actions
        :param update: (int) the current step iteration
        :param writer: (TensorFlow Summary.writer) the writer for tensorboard
        :param states: (np.ndarray) For recurrent policies,
            the internal state of the recurrent model
        ...
        if states is None:
            update_fac = (self.n_batch // self.nminibatches
                          // self.noptepochs + 1)
        else:
            update_fac = (self.n_batch // self.nminibatches //
                          self.noptepochs // self.n_steps + 1)

        if writer is not None:
            # run loss backprop with summary,
            # but once every 10 runs save the metadata
            # (memory, compute time, ...)
            if self.full_tensorboard_log and (1 + update) % 10 == 0:
                run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)
                run_metadata = tf.RunMetadata()
                (summary, policy_loss, value_loss,
                 policy_entropy, approxkl, clipfrac, _) = self.sess.run(
                    [self.summary, self.pg_loss, self.vf_loss,
                     self.entropy, self.approxkl, self.clipfrac, self._train],
                ...
                (summary, policy_loss, value_loss,
                 policy_entropy, approxkl, clipfrac, _) = self.sess.run(
                    [self.summary, self.pg_loss, self.vf_loss,
                     self.entropy, self.approxkl, self.clipfrac, self._train], td_map)
            writer.add_summary(summary, (update * update_fac))
        else:
            (policy_loss, value_loss, policy_entropy,
             approxkl, clipfrac, _) = self.sess.run(
                [self.pg_loss, self.vf_loss, self.entropy,
                 self.approxkl, self.clipfrac, self._train], td_map)
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1,
        ...
        tb_writer = TensorboardWriter(
            self.graph, self.tb_log, tb_log_name, new_tb_log)
        with SetVerbosity(self.verbose), tb_writer as writer:
            self._setup_learn()
            ...
            for update in range(1, n_updates + 1):
                assert self.n_batch % self.nminibatches == 0, (
                    "The number of minibatches (`nminibatches`) is not "
                    "a factor of the total number of samples "
                    "collected per rollout (`n_batch`), some samples won't be used.")
                    ...
                    for epoch_num in range(self.noptepochs):
                        ...
                        for start in range(0, self.n_envs, envs_per_batch):
                            timestep = self.num_timesteps // update_fac + (
                                    ...
                                    epoch_num * self.n_envs + start) // envs_per_batch
                            ...
                            mb_loss_vals.append(self._train_step(
                                ...
                                states=mb_states, cliprange_vf=cliprange_vf_now))
