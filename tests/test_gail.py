...
@pytest.mark.parametrize("expert_env", [
    ('Pendulum-v0', EXPERT_PATH_PENDULUM, True),
...
def test_gail(expert_env):
    ...
    dataset = ExpertDataset(
        traj_data=traj_data, expert_path=expert_path, traj_limitation=10, sequential_preprocessing=True)

    # Note: train for 1M steps to have a working policy
    model = GAIL(
        'MlpPolicy', env, adversary_entcoeff=0.0, lam=0.92, max_kl=0.001,
...
def test_generate(generate_env):
    ...
    dataset = generate_expert_traj(
        model, 'expert', n_timesteps=1000, n_episodes=n_episodes, image_folder='test_recorded_images')
...
def test_behavior_cloning_box(model_class):
    ...
    """
    dataset = ExpertDataset(
        expert_path=EXPERT_PATH_PENDULUM, traj_limitation=10, sequential_preprocessing=True, verbose=0)
...
def test_behavior_cloning_discrete(model_class):
    dataset = ExpertDataset(
        expert_path=EXPERT_PATH_DISCRETE, traj_limitation=10, sequential_preprocessing=True, verbose=0)
